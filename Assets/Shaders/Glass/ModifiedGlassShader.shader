Shader "ReV3nus/ModifiedGlass"
{
	Properties
	{
		...
		[Header(Refraction)]
		_ChromaticAberration("Chromatic Aberration", Range( 0 , 0.3)) = 0.043
		_Refraction("Refraction", Range( -2 , 4)) = 2.3
	...
	SubShader
	{
		...
		#include "../LiquidSimulator/Utils.cginc"
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			// Clipped from UnderWater
			float3 col = _Color.rgb;
			float clipArea = ClipArea(i.worldPos.xyz);
			float3 shallCol = lerp(float3(1, 1, 1), _ShallowColor.rgb, (1.0 - saturate((i.worldPos.y - _Range.x) / _Range.y))*clipArea);
			float3 deepCol = lerp(float3(1, 1, 1), _DeepColor.rgb, (1.0 - saturate((i.worldPos.y - _Range.z) / _Range.w))*clipArea);
			float3 caustic = SampleCaustic(i.worldPos.xyz, clipArea);
			col.rgb *= shallCol*deepCol;
			col.rgb *= UNITY_LIGHTMODEL_AMBIENT.rgb + (caustic + internalWorldLightColor.rgb + internalWorldLightColor.rgb);
			o.Albedo = col.rgb;
