    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if(_Depth > 0)
                {
                    depth = 1-Linear01Depth(tex2D(depthBuffer, flippedUVs)*255);
                    mix += _Depth*depth;
                }
                if(_Color > 0)
                {
                ...
                if(_Transparency > 0)
                {
                    transparency = 1-tex2D(_CurveTex, float2(tex2D(_GlobalTransparencyTexture, flippedUVs).r, 0)).r;
                    mix += _Transparency*transparency;
                }
                if(_Shadow > 0)
                {
                    
                    float cameraDepth = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture,flippedUVs));
